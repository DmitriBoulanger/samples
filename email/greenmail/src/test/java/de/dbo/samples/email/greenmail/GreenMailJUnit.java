...
public class GreenMailJUnit implements Users {
	...
	GreenMailSetUp setUp = new GreenMailSetUpBuilder()
		.withUser(USER1)
		.withUser(USER2)
		.withConfiguration(SERVER_SETUP_TO_READ, SERVER_SETUP_TO_SEND)
		.build();
	log.info(setUp.print().toString());
	setUp.start(400);
	
	GreenMailUtil.sendTextEmail(USER2.email, USER1.email, "Standalone test", "It worked", SERVER_SETUP_TO_SEND);	 
